...
export const ContextProvider = ({ children, rootPath, isLocalEnv }: Props) => {
  ...
  return <Context.Provider value={koksmat}>
    ...
    <Script
      id='beforeloadOffice'
      dangerouslySetInnerHTML={{ __html: 'window._replaceState = window.replaceState;window._history = window.history;' }} />
    ...
    {loadedOffice &&
      <Script
        id='afterloadOffice'
        dangerouslySetInnerHTML={{ __html: 'window.replaceState = window._replaceState;window.history = window._history;' }} />
    }
