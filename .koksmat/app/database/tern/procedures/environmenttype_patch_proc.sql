...
*/   
 ...
 CREATE OR REPLACE FUNCTION proc.patch_environmenttype(
...
BEGIN
    ...
    v_query := format('UPDATE public.environmenttype SET %s, updated_by = %L, updated_at = CURRENT_TIMESTAMP WHERE id = %L',
    ...
    IF v_rows_updated < 1 THEN
        RAISE EXCEPTION 'No records updated.  ID % not found in table environmenttype', p_id;
    END IF;
       p_auditlog_params := jsonb_build_object(
        'tenant', '',
        'searchindex', '',
        'name', 'patch_environmenttype',
        'status', 'success',
        'description', '',
        'action', 'patch_environmenttype',
