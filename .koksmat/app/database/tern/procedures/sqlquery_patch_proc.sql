...
*/   
 ...
 CREATE OR REPLACE FUNCTION proc.patch_sqlquery(
...
BEGIN
    ...
    v_query := format('UPDATE public.sqlquery SET %s, updated_by = %L, updated_at = CURRENT_TIMESTAMP WHERE id = %L',
    ...
    IF v_rows_updated < 1 THEN
        RAISE EXCEPTION 'No records updated.  ID % not found in table sqlquery', p_id;
    END IF;
