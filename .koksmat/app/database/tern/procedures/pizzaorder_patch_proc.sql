...
*/   
 ...
 CREATE OR REPLACE FUNCTION proc.patch_pizzaorder(
...
BEGIN
    ...
    v_query := format('UPDATE public.pizzaorder SET %s, updated_by = %L, updated_at = CURRENT_TIMESTAMP WHERE id = %L',
    ...
    IF v_rows_updated < 1 THEN
        RAISE EXCEPTION 'No records updated.  ID % not found in table pizzaorder', p_id;
    END IF;
       p_auditlog_params := jsonb_build_object(
        'tenant', '',
        'searchindex', '',
        'name', 'patch_pizzaorder',
        'status', 'success',
        'description', '',
        'action', 'patch_pizzaorder',
        'entity', 'pizzaorder',
        'entityid', -1,
        'actor', p_actor_name,
        'metadata', p_fields
