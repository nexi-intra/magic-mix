...
*/   
 ...
 CREATE OR REPLACE FUNCTION proc.patch_job(
...
BEGIN
    ...
    v_query := format('UPDATE public.job SET %s, updated_by = %L, updated_at = CURRENT_TIMESTAMP WHERE id = %L',
    ...
    IF v_rows_updated < 1 THEN
        RAISE EXCEPTION 'No records updated.  ID % not found in table job', p_id;
    END IF;
