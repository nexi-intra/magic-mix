...
*/   
 ...
 CREATE OR REPLACE FUNCTION proc.patch_processlog(
...
BEGIN
    ...
    v_query := format('UPDATE public.processlog SET %s, updated_by = %L, updated_at = CURRENT_TIMESTAMP WHERE id = %L',
    ...
    IF v_rows_updated < 1 THEN
        RAISE EXCEPTION 'No records updated.  ID % not found in table processlog', p_id;
    END IF;
       p_auditlog_params := jsonb_build_object(
        'tenant', '',
        'searchindex', '',
        'name', 'patch_processlog',
        'status', 'success',
        'description', '',
        'action', 'patch_processlog',
        'entity', 'processlog',
        'entityid', -1,
        'actor', p_actor_name,
        'metadata', p_fields
