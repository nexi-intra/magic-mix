...
*/   
 ...
 CREATE OR REPLACE FUNCTION proc.patch_session(
...
BEGIN
    ...
    v_query := format('UPDATE public.session SET %s, updated_by = %L, updated_at = CURRENT_TIMESTAMP WHERE id = %L',
    ...
    IF v_rows_updated < 1 THEN
        RAISE EXCEPTION 'No records updated.  ID % not found in table session', p_id;
    END IF;
