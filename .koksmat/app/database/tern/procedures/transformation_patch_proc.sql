...
*/   
 ...
 CREATE OR REPLACE FUNCTION proc.patch_transformation(
...
BEGIN
    ...
    v_query := format('UPDATE public.transformation SET %s, updated_by = %L, updated_at = CURRENT_TIMESTAMP WHERE id = %L',
    ...
    IF v_rows_updated < 1 THEN
        RAISE EXCEPTION 'No records updated.  ID % not found in table transformation', p_id;
    END IF;
       p_auditlog_params := jsonb_build_object(
        'tenant', '',
        'searchindex', '',
        'name', 'patch_transformation',
        'status', 'success',
        'description', '',
        'action', 'patch_transformation',
