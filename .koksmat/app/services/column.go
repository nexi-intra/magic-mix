...
func HandleColumnRequests(req micro.Request) {
...
if (len(payload.Args) < 2) {
    ...
    return
            ...
            if err != nil {
                log.Println("Error", err)
                ServiceResponseError(req, "Error unmarshalling column")
                return
            }
                     
    result,err := column.ColumnCreate(object)
    if (err != nil) {
        log.Println("Error", err)
        ServiceResponseError(req, fmt.Sprintf("Error calling ColumnCreate: %s", err))
...
if (len(payload.Args) < 2) {
    ...
    if (err != nil) {
        log.Println("Error", err)
        ServiceResponseError(req, fmt.Sprintf("Error calling ColumnUpdate: %s", err))
...
if (len(payload.Args) < 2) {
    ...
    result,err := column.ColumnSearch(payload.Args[1])
    if (err != nil) {
        log.Println("Error", err)
        ServiceResponseError(req, fmt.Sprintf("Error calling ColumnSearch: %s", err))
